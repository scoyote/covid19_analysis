...
%macro LoopShowChanges;
	PROC SQL ;
		select count( distinct statename) into :numstates from _statelist;
		select statename into :st1-:st%cmpres(&numstates) from _statelist;
	quit;
	%let numstates=%cmpres(&numstates);

	%do i=1 %to &numstates;
		data _null_;                                                                                                                   
          call symput("titleinclude",fipnamel(stfips("&&st&i")));                                                                                                                            
		run;
		%let titleinclude=%cmpres(&titleinclude);
		%put NOTE: Running on &i of &numstates &&st&i - &titleinclude;
/* %end;	 */
/* %mend;	 */
...
/* %loopshowchanges; */
		...
		data _plotdata _summarydata;
			set &level._trajectories(where=(substr(cbsa_title,length(cbsa_title)-1,length(cbsa_title)) ="&&st&i" and &analysisvar>0));
			by &catvar. filedate;
		...
		%InsertPDFReportHeader(style=styles.htmlblue,fname=&level._&titleinclude,titlespec=&titleinclude. Directional);
