...
%CreateModelData(FIPS,FIPS,%str(48029),startdate=01feb20,enddate=&rundate,back=&back);
...
ods html5 body="SEIRModel_&filepart..htm" path='graphs/' options(svg_mode="inline" bitmap_mode='inline');
...
data _cmap;
	informat id $20. value $1. linecolor markercolor markersymbol $30. color $50. markersize markertransparency 8.;
	ID='roles';value = 'I'; color	= 'green' ; markersize=10; markertransparency=0.5; markersymbol='circle'; 	markercolor=color; linecolor=color; fillcolor=color; linethickness=0;output;
	ID='roles';value = 'O'; color	= 'blue'; 	markersize=20; markertransparency=0.5; markersymbol='triangle';	markercolor=color; linecolor=color; fillcolor=color; linethickness=0;output;
	ID='roles';value = 'E'; color	= 'BWH';  	markersize=20; markertransparency=0.5; markersymbol='square';	markercolor=color; linecolor=color; fillcolor=color; linethickness=0;output;
run;
	...
	proc sgplot data=_graph des="SEIR Short Term Forecast" dattrmap=_cmap ;
		...
		series x=date y=actual_cumulative_cases / group=role  name="Actual Cases" markers attrid=roles;
		xaxis interval=day  ;
