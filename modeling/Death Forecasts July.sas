...
%let windowback=7;
%let cbsa=Miami-Fort Lauderdale-Pompano Beach, FL;
...
proc ARIMA data=_modeldata(where=(filedate>'01mar20'd));
	...
	estimate noprint  p=(1,7) q=(2) method=ml; run;	
	/* fit prewhitened series */
	identify noprint var = dailydeaths crosscorr=(dailycases) scan esacf p=(0:7) q=(0:7) nlag=60 ; run;
	/* estimate ccf */
	estimate  p=(7) q=(7) input=(12 $ /(7) dailycases) method=ml;run;
	forecast id=filedate noprint out=_forecast back=7 lead=7 ;
quit;
