...
/* by province_state; */
/* 	identify &np var = dailycases(1,7) scan esacf p=(0:7) q=(0:7) stationarity=(adf=7 dlag=2); run; */
/* 	estimate &np p=(2 3 7) q=1 noint  method=ml; run;	 */
/* 	fit prewhitened series */
/* 	identify  var = dailydeaths crosscorr=(dailycases) scan esacf  p=(0:7) q=(0:7) nlag=60 ; run; */
/* 	estimate ccf */
/* 	estimate  p=(1 6 7) q=0 noint input=(14 $ /(7) dailycases) method=ml;run; */
/* 	forecast id=filedate noprint out=_forecast back=0 lead=42 ; */
/* quit; */
proc ARIMA data=_modeldata(where=(filedate>'01mar20'd));
by province_state;
	identify &np var = dailycases scan esacf p=(0:7) q=(0:7) stationarity=(adf=7 dlag=2); run;
	estimate &np p=(2 3 7) q=0  noint method=ml; run;	
	/* fit prewhitened series */
	
	identify  var = dailydeaths crosscorr=(dailycases) scan esacf  p=(0:7) q=(0:7) nlag=60 ; run;
	/* estimate ccf */
	estimate  p=(1 6 7) q=0 noint input=(21 $ /(7) dailycases) method=ml;run;
	forecast id=filedate noprint out=_forecast back=0 lead=42 ;
quit;
