...
proc ARIMA data=_modeldata(where=(filedate>'01apr20'd));
	by cbsa_title;
	identify noprint var = dailycases(1,7) scan esacf ; run;
	estimate noprint p=1 q=1 noint method=ml; run;	
	/* fit prewhitened series */
	identify var = dailydeaths crosscorr=(dailycases) scan esacf p=(0:7) q=(0:7) nlag=60 ; run;
	/* estimate ccf */
	estimate noint p=1 q=0 input=(26 $ /(7) dailycases) method=ml;run;
