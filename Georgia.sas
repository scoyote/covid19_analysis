...
*****************************;
%let region_name=Georgia;
*****************************;

data &region_name;
	set  WORK.JHU_final;
	where Province_State = "&region_name";
run;
...
title Georgia COVID-19 Situation Report;
...
proc sgplot data=&region_name._summary nocycleattrs;
	...
	yaxis grid min=&respmin1 max=&respmax1 %offset();* label="Number of Confirmed Infections";
	y2axis min=&respmin2 max=&respmax2 %offset();* label="Number of Deaths";
